    ...
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        if (result is Result r && r.IsFailure)
        {
            using (LogContext.PushProperty("Error", result.Error, true))
            {
                ...
                _logger.LogError("[FAILURE] {RequestType} {RequestName} failed with error {Error}, Params: {Params}, at {DateTimeUtc}",
                    requestTypeLabel, requestName, result.Error, parametersString.ToString(), DateTime.UtcNow);
            }
        }
        else
        {
